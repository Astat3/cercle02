...
static void	target_a(t_stack *a, t_stack *b) // what if we send twice INT MIN ?
{
	t_stack *target_node;
	t_stack *current_b;
	long	current_match;
	...
	while (a)
	{
		...
		current_b = b; // similaire à j = 0; || le pointeur ne se remet pas à 0 donc il faut utiliser une variable tmp;
		while (current_b)
		{
...
void	cheap(t_stack *a) // give the cheapest node value
{
...
{
	index_in_stack(a); //get all the index and position on the stack | above / below the median
	index_in_stack(b);
	target_a(a, b);
	how_much(a, b);
	cheap(a);
