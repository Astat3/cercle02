...
/*   Updated: 2025/12/10 00:58:48 by agallot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	is_pixel(int x, int y, t_fractal *fractal)
{
	...
	z.x = (fractal->zoom * scale(x, -2, +2, WIDTH)) + fractal->shift_x;
	z.y = (fractal->zoom * scale(y, +2, -2, HEIGHT)) + fractal->shift_y;
	i = 0;
	juliadelbrot(&z, &c, fractal);
	while (i < fractal->interation)
	{
		z = sum_vectors(product_vector(z), c);
		if ((z.x * z.x) + (z.y * z.y) > fractal->escape)
		{
			color = scale(i, BLACK, RED, fractal->interation);
